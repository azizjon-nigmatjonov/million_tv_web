        ...
        >
            <div className="text-left h-auto transition duration-300 ease-in-out transform cursor-pointer md:hover:scale-105 hover:shadow group relative">
                ...
                >
                    <div
                        className={` ${
                            fullWidth ? 'w-full' : 'w-[317px]'
                        } relative  ${
                            additionalHeight ? additionalHeight : 'h-[200px]'
                        }`}
                    >
                        {el?.is_megogo && !el?.related_movies && (
                            <div className="bg-mainColor w-[28px] h-[28px] z-[2] rounded-[4px] absolute left-2 top-2 text-white uppercase text-[13px] font-semibold flex items-center justify-center">
                                <MegaGoTag />
                            </div>
                        )}
                        {el?.is_premier && !el?.related_movies && (
                            <div className="bg-mainColor w-[28px] h-[28px] z-[2] rounded-[4px] absolute left-2 top-2 text-white uppercase text-[14px] font-semibold flex items-center justify-center">
                                <PremierTag />
                            </div>
                        )}
                        <LazyLoadImage
                            ...
                            effect="blur"
                            delayTime={10000}
                            className={`rounded-t-[8px] object-cover w-full ${
                                additionalHeight
                                    ? additionalHeight
                                    : 'h-[200px]'
                            }`}
                    ...
                    <div
                        className={`z-[2] relative rounded-b-[8px] h-[70px] overflow-hidden bg-secondryBackground p-3 flex justify-between flex-col  ${
                            fullWidth ? 'w-full' : 'w-[317px] md:w-[317px]'
                        }`}
                    >
                        <span className="text-white font-[600] whitespace-nowrap overflow-hidden">
                            {i18n?.language === 'en'
                                ? el?.title?.en
                                : i18n?.language === 'ru'
                                ? el?.title?.ru
